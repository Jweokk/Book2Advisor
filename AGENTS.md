...
用户要编译人物模型时，按以下顺序判定（不要问用户，除非条件冲突）：

| 条件 | 选择 | 理由 |
|---|---|---|
| 有 LLM key（`.env` 的 DEEPSEEK_API_KEY，或已设 LLM_BASE_URL/LLM_MODEL）且用户未指定方式 | **快速路径**（`extract_candidates.py` → `merge_candidates.py`） | 脚本自动调 LLM、分段/分批/校验全自动，省事且稳 |
| **无任何 LLM key** | **自主蒸馏**（用自己的能力提取/融合，脚本只做校验/导出） | 快速路径无法运行；机械步骤仍用仓库脚本 |
| 用户明确说"用自己的模型 / 自己的 agent / 不要 DeepSeek" | **自主蒸馏** | 尊重用户工具链（详见 docs/AGENT-DISTILLATION.md） |
| 用户明确说"跑脚本 / 自动化 / 快速出结果" | **快速路径** | 明确指令优先 |
| 语料 >30 篇或用户要求高质量骨架 | **快速路径 + 人工审核提示** | 脚本分批/分段更稳，但交付时提示人工审核 tensions/evolution 与 quote 抽样 |

判定后若走快速路径但缺少 key：**不要报错中断**——自动降级为自主蒸馏并告知用户"未检测到 LLM key，改用本 agent 自主蒸馏（脚本仍做校验/导出）"。
